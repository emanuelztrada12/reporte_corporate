...
$rspta=$reporte1->reportegeneralTwo($inicio, $fin, $curso, $vp );
        $data= Array(); 
          foreach ($rspta as $key => $valor) {
            // TODO: cambiar enlace por el sitio original
            $link = "<a href='http://localhost:8080/blocks/report/vistas/tablerouser.php?blockid=13&courseid=1' target='_blank'>Visualizar lista</a>";
              $data[]=array(
                "0"=>$valor->id,
                "1"=>$valor->rolicasa,
